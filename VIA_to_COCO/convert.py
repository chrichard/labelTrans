...
def convert(imgdir, annpath):
    ...
    for img_id, key in enumerate(ann.keys()):
        ...
        for region in regions:
            ...
            cat = region['region_attributes']['road'] #我的返回的子类的编号，所以其实不需要这一段，
            print(cat)
...
if __name__== '__main__':
    img_path = '/tao/maskrcnn/data/raw-data/validating_data/' #改成自己的图片路径
    anno_path = '/tao/maskrcnn/data/raw-data/validating_data/via_region_data_rider.json' #自己的标注文件的路径。注意这里不是使用的VIA导出的coco格式文件，而是单纯的json格式文件。
    result_path = 'coco_val.json' #输出，结果文件
