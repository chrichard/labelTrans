...
ratio=1 #图像缩小0.5
labelme_images = '/AI/DLDataSet/cityscape/cityscape_labelme/train2017'
...
class labelme2coco(object):
    ...
    def data_transfer(self):
        for num, json_file in enumerate(self.labelme_json):
            with open(json_file, "r") as fp:
                print(num," of  ",len(labelme_json),"  ", json_file.split("/")[-1])
                data = json.load(fp)
    ...
    def image(self, data, num,json_file):
        ...
        imgfile = imgfile.split("_gtCoarse_polygons.json")[0]+"_leftImg8bit.png"
        #读取图片，然后缩放
        img_org= PIL.Image.open(labelme_images+"/"+imgfile)
        img_resize=img_org.resize(( self.width, self.height,))

        imgfile_resize = imgfile.split("_gtCoarse_polygons.json")[0]+".jpg"
...
if __name__ == "__main__":
    ...
    '''
    output='/AI/DLDataSet/cityscape/cityscape_labelme/annotations/instances_train2017.json'
